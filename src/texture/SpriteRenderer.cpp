...
void SpriteRenderer::DrawSprite(Texture2D &texture, glm::vec2 position, glm::vec2 size, float rotate, glm::vec3 color, float aspectRatio)
{
	...
	model = glm::translate(model, glm::vec3(position.x-(size.x/2), position.y-(size.y/2), 0.0f));  // first translate (transformations are: scale happens first, then rotation, and then final translation happens; reversed order)
